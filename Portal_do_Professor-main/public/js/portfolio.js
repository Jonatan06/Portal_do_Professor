document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- CONFIGURAÇÃO FINAL E CORRIGIDA DO TINYMCE ---
    tinymce.init({
        ...
        // Manipulador de upload baseado em Promise com a API Fetch
        images_upload_handler: (blobInfo) => new Promise((resolve, reject) => {
            ...
            if (!token) {
                reject('Token de autenticação não encontrado.');
                return;
            }

            // Usa fetch para fazer o upload
            fetch('/api/projetos/upload-image', {
                method: 'POST',
                headers: {
                    'x-auth-token': token // Adiciona o cabeçalho de autenticação
                },
                body: formData
            })
            .then(response => {
            ...
            .then(json => {
                if (!json || typeof json.location !== 'string') {
                    reject('Resposta JSON inválida do servidor.');
                    return;
                }
                resolve(json.location);
            })
            .catch(err => {
                reject(`Falha no upload da imagem: ${err.message}`);
            });
        }),
    ...
    function createProjectHTML(data) {
        ...
        // Correção aplicada aqui, adicionando a classe "btn-icon"
        const detailLink = `<a href="/portfolio-detalhe?id=${data.id}" target="_blank" class="btn-icon" data-tooltip="Ver página do projeto"><i class="fa-solid fa-eye"></i></a>`;
