document.addEventListener('DOMContentLoaded', async () => {
    ...
    let currentUser = {};
    let professorImageUrl = '/uploads/images/default-avatar.png'; // Valor padrão
    ...
    // NOVO: Busca dados do perfil do professor (incluindo a foto)
    async function fetchProfessorProfile() {
        try {
            const response = await fetch('/api/public-profile');
            if (response.ok) {
                const profile = await response.json();
                if (profile.imagem_url) {
                    professorImageUrl = profile.imagem_url;
                }
            }
        } catch (error) {
            console.error("Não foi possível carregar a foto do professor:", error);
        }
    }
            ...
            postContainer.innerHTML = `
                <header class="post-header">
                    ...
                    <div class="post-meta-author">
                        <img src="${professorImageUrl}" alt="Foto do Professor" class="author-photo">
    ...
    async function init() {
        ...
        await fetchProfessorProfile(); // Busca a foto do professor primeiro
        await fetchPostDetails(); // Depois renderiza o post já com a foto
        await fetchAndRenderComments();
    }
